...
int main() {
    ...
    while (st.size()) {
        int i, j;
        tie(i, j) = pos(st.begin() -> S);
        st.erase(st.begin());
        forn(dir, 0, 3) {
            int ni = i + di[dir], nj = j + dj[dir];
            if (!in(ni, nj) || a[ni][nj] == '#') continue;
            if (d[ni][nj] > d[i][j] + 1) {
                st.erase({d[ni][nj], id(ni, nj)});
                d[ni][nj] = d[i][j] + 1;
                st.insert({d[ni][nj], id(ni, nj)});
            }
            if (!to[i][j][dir]) continue;
            ni = i + to[i][j][dir] * di[dir], nj = j + to[i][j][dir] * dj[dir];
            if (d[ni][nj] > d[i][j] + bd[i][j]) {
                st.erase({d[ni][nj], id(ni, nj)});
                d[ni][nj] = d[i][j] + bd[i][j];
                st.insert({d[ni][nj], id(ni, nj)});
            }
        }
    }
